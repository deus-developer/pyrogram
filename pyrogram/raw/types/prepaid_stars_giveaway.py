...
class PrepaidStarsGiveaway(TLObject):  # type: ignore
    ...
    __slots__: list[str] = ["boosts", "date", "id", "quantity", "stars"]

    ID = 0x9A9D77E0
    QUALNAME = "types.PrepaidStarsGiveaway"

    def __init__(
        self, *, id: int, stars: int, quantity: int, boosts: int, date: int,
    ) -> None:
        self.id = id  # long
    ...
    def read(b: BytesIO, *args: Any) -> "PrepaidStarsGiveaway":
        ...
        return PrepaidStarsGiveaway(
            id=id, stars=stars, quantity=quantity, boosts=boosts, date=date,
        )
