...
class KeyboardButtonUrlAuth(TLObject):  # type: ignore
    ...
    __slots__: list[str] = ["button_id", "fwd_text", "text", "url"]

    ID = 0x10B78D29
    QUALNAME = "types.KeyboardButtonUrlAuth"

    def __init__(
        self, *, text: str, url: str, button_id: int, fwd_text: str | None = None,
    ) -> None:
    ...
    def read(b: BytesIO, *args: Any) -> "KeyboardButtonUrlAuth":
        ...
        return KeyboardButtonUrlAuth(
            text=text, url=url, button_id=button_id, fwd_text=fwd_text,
        )
