...
class Message(Object, Update):
    ...
    def __init__(
        ...
        *,
        ...
        giveaway_launched: bool | None = None,
        chat_ttl_period: int | None = None,
        boosts_applied: int | None = None,
        join_request_approved: bool | None = None,
    ...
    async def from_raw_tl(
        ...
        if isinstance(message, raw.types.MessageService):
            ...
            if isinstance(action, raw.types.MessageActionChatAddUser):
                new_chat_members = [
                    types.User.from_raw_tl(
                        client,
                        client.entity_cache.get_user(user_id=i),
                    )
                ...
                new_chat_members = [
                    types.User.from_raw_tl(
                        client,
                        client.entity_cache.get_peer(peer=message.from_id),
                    ),
                ]
                service_type = enums.MessageServiceType.NEW_CHAT_MEMBERS
            elif isinstance(action, raw.types.MessageActionChatDeleteUser):
                left_chat_member = types.User.from_raw_tl(
                    client,
                    client.entity_cache.get_user(user_id=action.user_id),
                )
                ...
                video_chat_members_invited = types.VideoChatMembersInvited.from_raw_tl(
                    client,
                    action,
                )
            ...
            sender_chat = (
                types.Chat.from_raw_tl(client, message, is_chat=False)
                if not from_user
                else None
            )
            ...
            if isinstance(action, raw.types.MessageActionGameScore):
                parsed_message.game_high_score = types.GameHighScore.from_raw_tl_action(
                    client,
                    message,
                )
        ...
        if isinstance(message, raw.types.Message):
                ...
                if forward_header.from_id:
                    ...
                    if peer_id > 0:
                        forward_from = types.User.from_raw_tl(
                            client,
                            client.entity_cache.get_peer(peer=forward_header.from_id),
                        )
                    else:
            ...
            if media:
                if isinstance(media, raw.types.MessageMediaPhoto):
                    ...
                    giveaway_result = await types.GiveawayResult.from_raw_tl(
                        client,
                        media,
                    )
                    ...
                    if media.story:
                        story = await types.Story.from_raw_tl(
                            client,
                            media.story,
                            media.peer,
                        )
                        ...
                        try:
                            story = await client.get_stories(
                                utils.get_peer_id(media.peer),
                                media.id,
                            )
                    ...
                    if isinstance(doc, raw.types.Document):
                        ...
                        if raw.types.DocumentAttributeAnimated in attributes:
                            ...
                            if video_attributes.round_message:
                                video_note = types.VideoNote.from_raw_tl(
                                    client,
                                    doc,
                                    video_attributes,
                                    media.ttl_seconds,
                            ...
                            document = types.Document.from_raw_tl(
                                client,
                                doc,
                                file_name,
                            )
            ...
            parsed_message = Message(
                ...
                via_bot=types.User.from_raw_tl(
                    client,
                    client.entity_cache.get_user(user_id=message.via_bot_id),
                ),
            ...
            if message.reply_to:
                if isinstance(message.reply_to, raw.types.MessageReplyHeader):
                    if message.reply_to.forum_topic:
                        ...
                        if topics:
                            parsed_message.topic = types.ForumTopic.from_raw_tl(
                                client,
                                topics[thread_id],
                            )
                    else:
                    ...
                    if parsed_message.reply_to_message_id:
                        ...
                        try:
                            ...
                            if not reply_to_message:
                                with contextlib.suppress(ChannelPrivate):
    ...
    async def click(
        ...
        if isinstance(x, int) and y is None:
            try:
                [button for row in keyboard for button in row][x]
            except IndexError:
