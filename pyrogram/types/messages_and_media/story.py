...
class Story(Object, Update):
    ...
    async def from_raw_tl(
        ...
        if forward_header:
            ...
            if fwd_peer_id > 0:
                forward_from = types.User.from_raw_tl(
                    client,
                    client.entity_cache.get_peer(peer=forward_header.from_peer),
                )
            else:
                forward_from_chat = types.Chat.from_raw_tl_channel_chat(
                    client,
                    client.entity_cache.get_peer(peer=forward_header.from_peer),
                )
