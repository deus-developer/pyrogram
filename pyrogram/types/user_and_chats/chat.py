...
class Chat(Object):
    ...
    def from_raw_tl_chat_chat(client, chat: raw.types.Chat) -> "Chat":
        ...
        return Chat(
            ...
            usernames=types.List([types.Username.from_raw_tl(r) for r in usernames])
            or None,
            photo=types.ChatPhoto.from_raw_tl(
                client,
            ...
            permissions=types.ChatPermissions.from_raw_tl(
    ...
    def from_raw_tl_channel_chat(client, channel: raw.types.Channel) -> "Chat":
        ...
        return Chat(
            ...
            restrictions=types.List(
                [types.Restriction.from_raw_tl(r) for r in restriction_reason],
            )
            or None,
            permissions=types.ChatPermissions.from_raw_tl(
            ...
            profile_color=types.ChatColor.from_raw_tl(
    ...
    def from_raw_tl(
        ...
        if isinstance(message.peer_id, raw.types.PeerUser):
            return Chat.from_raw_tl_user_chat(
                client,
                client.entity_cache.get_user(user_id=chat_id),
            )

        if isinstance(message.peer_id, raw.types.PeerChat):
            return Chat.from_raw_tl_chat_chat(
                client,
                client.entity_cache.get_chat(chat_id=chat_id),
            )

        return Chat.from_raw_tl_channel_chat(
            client,
            client.entity_cache.get_channel(channel_id=chat_id),
        )
    ...
    def from_raw_tl_dialog(client, peer):
        if isinstance(peer, (raw.types.PeerUser, raw.types.InputPeerUser)):
            return Chat.from_raw_tl_user_chat(
                client,
                client.entity_cache.get_user(user_id=peer.user_id),
            )
        if isinstance(peer, (raw.types.PeerChat, raw.types.InputPeerChat)):
            return Chat.from_raw_tl_chat_chat(
                client,
                client.entity_cache.get_chat(chat_id=peer.chat_id),
            )
        return Chat.from_raw_tl_channel_chat(
            client,
            client.entity_cache.get_channel(channel_id=peer.channel_id),
        )

    @staticmethod
    async def from_raw_tl_full(
        client,
        ...
        if isinstance(chat_full, raw.types.users.UserFull):
            ...
            parsed_chat = Chat.from_raw_tl_user_chat(
                client,
                client.entity_cache.get_user(user_id=full_user.id),
            )
            parsed_chat.bio = full_user.about
            parsed_chat.folder_id = getattr(full_user, "folder_id", None)
            parsed_chat.business_info = types.BusinessInfo.from_raw_tl(
                client,
                full_user,
            )
            parsed_chat.business_intro = await types.BusinessIntro.from_raw_tl(
                client,
                getattr(full_user, "business_intro", None),
            )
            parsed_chat.birthday = types.Birthday.from_raw_tl(
            ...
            if full_user.personal_channel_id:
                parsed_chat.personal_channel = Chat.from_raw_tl_channel_chat(
                    client,
                    client.entity_cache.get_channel(
            ...
            if full_user.wallpaper and isinstance(
                ...
                parsed_chat.wallpaper = types.Document.from_raw_tl(
                    client,
            ...
            if isinstance(full_chat, raw.types.ChatFull):
                ...
                linked_chat_raw = client.entity_cache.get_channel(
                    channel_id=full_chat.linked_chat_id,
                )

                if linked_chat_raw:
                    parsed_chat.linked_chat = Chat.from_raw_tl_channel_chat(
                        client,
                        linked_chat_raw,
                    )
                ...
                if default_send_as:
                    ...
                    parsed_chat.send_as_chat = Chat.from_raw_tl_chat(
                        client,
                        send_as_raw,
                    )
            ...
            parsed_chat.available_reactions = types.ChatReactions.from_raw_tl(
                client,
