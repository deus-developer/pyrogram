...
async def ainput(prompt: str = "", *, hide: bool = False):
    ...
    with ThreadPoolExecutor(1) as executor:
        func = getpass if hide else input
        return await asyncio.get_event_loop().run_in_executor(executor, func, prompt)
...
def pack_inline_message_id(msg_id: "raw.base.InputBotInlineMessageID") -> str:
...
def get_reply_to(
    ...
    quote_entities: types.List[raw.base.MessageEntity] | None = None,
...
def compute_password_check(
    ...
    B_bytes = r.srp_B  # noqa: N806
    B = btoi(B_bytes)  # noqa: N806
    ...
    while True:
        ...
        A = pow(g, a, p)  # noqa: N806
        A_bytes = itob(A)  # noqa: N806
    ...
    ux = u * x
    a_ux = a + ux
    S = pow(g_b, a_ux, p)  # noqa: N806
    S_bytes = itob(S)  # noqa: N806

    K_bytes = sha256(S_bytes)  # noqa: N806

    M1_bytes = sha256(  # noqa: N806
...
) -> dict[str, str | types.List["raw.base.MessageEntity"]]:
    if entities:
...
def parse_text_with_entities(
    ...
    str,
    Str | types.List[types.MessageEntity] | None,
]:
    entities = types.List(
        filter(
...
def expand_inline_bytes(bytes_data: bytes) -> bytearray:
