...
from pyrogram.errors import (
    AuthKeyDuplicated,
    BadMsgNotificationError,
    FloodPremiumWait,
    FloodWait,
    InternalServerError,
    RPCError,
    SecurityCheckMismatchError,
    ServiceUnavailable,
    Unauthorized,
)
...
class Session:
    ...
    def __init__(
        ...
        self.ipv6 = self.client.ipv6
        self.proxy = self.client.proxy
        self.protocol_factory = self.client.protocol_factory

        self.crypto_executor = self.client.crypto_executor
    ...
    async def handle_packet(self, packet) -> None:
        ...
        for msg in messages:
            ...
            try:
                ...
                if self.stored_msg_ids:
                    if msg.msg_id < self.stored_msg_ids[0]:
                        raise SecurityCheckMismatchError(
            ...
            except SecurityCheckMismatchError as e:
    ...
    async def send(
        ...
        if isinstance(result, raw.types.BadMsgNotification):
            log.warning(
                "%s: %s",
                BadMsgNotificationError.__name__,
                BadMsgNotificationError(result.error_code),
            )
    ...
    async def invoke(
        ...
        while True:
            try:
                ...
                log.warning(
                    'Waiting for %s seconds before continuing (required by "%s")',
                    amount,
                    query_name,
                )
